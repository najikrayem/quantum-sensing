...
/*
 * This function sets up one of the RP2350’s hardware PWM slices to produce a repeating
 * waveform at the specified frequency. It does this by calculating the required “wrap”
 * value — the number of system clock cycles between each PWM period — and programming
 * it into the PWM hardware. The slice is then enabled so it begins running.
 *
 * Each PWM slice controls two output channels (A and B) that can share the same frequency
 * but have independent duty cycles. This function only sets the frequency and starts
 * the slice; it does not assign GPIO pins or adjust duty cycles.
 *
 * To actually output a PWM signal, you must:
 *  1. Connect a GPIO pin to this slice using gpio_set_function(pin, GPIO_FUNC_PWM).
 *  2. Set the duty cycle for channel A or B using pwm_set_chan_level(slice, channel, level).
 *
 * */
